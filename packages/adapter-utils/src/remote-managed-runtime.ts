...
export async function prepareRemoteManagedRuntime(input: {
  ...
  // Policy is needed for the per-asset capacity gates further down, which run
  // regardless of syncWorkspace — so resolve it unconditionally.
  const workspacePolicy = resolveSshRemoteWorkspacePolicy();
  // The WORKSPACE capacity gate only applies when we are actually syncing a workspace;
  // upstream's syncWorkspace opt-out skips the whole prep, so there is nothing to reserve for.
  if (syncWorkspace) {
