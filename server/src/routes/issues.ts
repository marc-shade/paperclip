...
export function issueRoutes(
  ...
  router.get("/companies/:companyId/issues", async (req, res) => {
    ...
    const listFilters: IssueFilters = {
      ...
      // upstream added a parentIssueId alias; keep it, but through the uuid validator
      // so a malformed value still 422s instead of surfacing a Postgres 22P02 as a 500.
      parentId: uuidFilters.parentId ?? uuidFilters.parentIssueId,
  ...
  router.patch("/issues/:id", validate(updateIssueRouteSchema), async (req, res) => {
    ...
    try {
      // ARC-5774: reclaim stale execution locks before attempting the update, so a fresh
